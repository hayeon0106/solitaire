...
public class csPlay : MonoBehaviour
{
	...
	bool cardPull()
	{
		...
		//Debug.Log("pullSuccess = "+pullSuccess);
		//카드가 무조건 하나 이상 있어야 함
		for(int i=0; i<cardList.Length; i++)
		{
			//Debug.Log("cardList 크기 = " + cardList[i].getListSize());
		...
		else
		{
			for (int i = 0; i < cardList.Length; i++)
			{
				...
				y = cardList[i].getList()[cardList[i].getList().Count - 1].getCard().transform.position.y - 1.0f;  //리스트의 마지막 카드의 y위치 - 1.0f
				...
				//로그
				//Debug.Log("뿌리는 카드 Y = "+tmpDeck[i].getY()+"tmpDeck[i] = "+tmpDeck[i]);
				//Debug.Log("cardPull\n 마지막 카드 인덱스: " + (l.Count - 1));
				//Debug.Log("cardPull_카드위치\n마지막 카드의 y = " + l[l.Count - 1].getY() + " 개수 * 0.5 = " + tmpList.getListSize() * -0.5f);
				//Debug.Log("cardPull_위치\ny = " + y + " z = " + z);
